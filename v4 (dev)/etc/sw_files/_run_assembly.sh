...
echo "LAUNCHING META-PIPE ASSEMBLY..."
...
while [[ $ASSEMBLY_NODE != *"$CLUSTER_NAME"* ]]; do
    for (( i=1; i<=${#WORKER_HOSTS[*]}; i++ )); do
        if [[ $(sed $i'q;d' $METAPIPE_MAIN_DIR/metapipe-tmp/assembly_running) == "0" ]]; then
            ASSEMBLY_NODE=${WORKER_HOSTS[$i]}
            ASSEMBLY_NODE_INDEX=$i
            sed -i $i"s/.*/1/" $METAPIPE_MAIN_DIR/metapipe-tmp/assembly_running
            break
        fi
        echo "0" >> $METAPIPE_MAIN_DIR/metapipe-tmp/assembly_running
    done
...
ssh -n cloud-user@$ASSEMBLY_NODE "
    ...
    java -jar $METAPIPE_MAIN_DIR/workflow-assembly-0.1-SNAPSHOT.jar \
    assembly -1 $ARG1 -2 $ARG2 \
    --config-file $METAPIPE_MAIN_DIR/.metapipe/conf.json \
    --attempt-id $(date +%s) \
    --num-threads $CORES_PER_EXECUTOR
"

sed -i $ASSEMBLY_NODE_INDEX's/.*/0/' $METAPIPE_MAIN_DIR/metapipe-tmp/assembly_running
