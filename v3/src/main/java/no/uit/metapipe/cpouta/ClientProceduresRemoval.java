...
class ClientProceduresRemoval
{
    ...
    static void deleteCluster(JSch ssh, /*ServerGroupApi serverGroupApi, */Configuration config, Server bastion, Server master,
                              String authCommands, VolumeApi volumeApi, VolumeAttachmentApi volumeAttachmentApi)
    {
        System.out.println("Starting cluster removing procedure...");
        String commands;
        if(Utils.objectHasContents(config.getSwDiskID()) && volumeApi.get(config.getSwDiskID()) != null && master != null)
        {
            commands = "source " + config.getXternFiles().get("diskHelper") + " " + config.getSwDiskName() + " unmount;" ;
...
//        }
                              ...
                              VolumeAttachmentApi volumeAttachmentApi, Server bastion)
    {
        ...
        String commands = "source " + config.getXternFiles().get("diskHelper") + " " + config.getSwDiskName() + " unmount;" ;
        Utils.sshExecutor(ssh, config.getUserName(), Utils.getServerPublicIp(bastion, config.getNetworkName()), commands);
