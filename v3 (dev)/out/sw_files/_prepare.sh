...
if [ "$2" != "skip-unpack" ]; then
    sudo tar xvf /media/$DISK_NAME/$METAPIPE_ON_SWDISK_DIR_NAME/metapipe-dependencies.tar.gz -C $METAPIPE_DIR --keep-old-files
    sudo mv package/dist .
    sudo mv dist metapipe
fi
sudo cp /media/$DISK_NAME/$METAPIPE_ON_SWDISK_DIR_NAME/*.sh $METAPIPE_DIR
#sudo cp /media/$DISK_NAME/$METAPIPE_ON_SWDISK_DIR_NAME/conf.json $METAPIPE_DIR
sudo cp /media/$DISK_NAME/$METAPIPE_ON_SWDISK_DIR_NAME/$METAPIPE_EXECUTABLE $METAPIPE_DIR
