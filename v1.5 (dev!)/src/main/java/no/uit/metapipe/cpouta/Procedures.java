...
//    }
            ...
            for(int i = 0; i < lines.size(); )
            {
                ...
                else if(line.contains("master:"))
                {
                    ...
                    for(j = i+1; !line.contains("node_groups:"); )
                    {
                        ...
                        {
                            newLines.add(Utils.updateFileLine(line, "image", config.getMaster().get("image")));
                            j++;
                        }
                ...
                else if(line.contains("disk:"))
                {
                    if(config.getNodeGroups().contains("regularHddNodes"))
                    {
                        ...
                        for(j = i+1; !line.contains("ssd:"); j++)
                        {
                            ...
                            {
                                newLines.add(Utils.updateFileLine(line, "image", config.getRegularHddNodes().get("image")));
                            }
                ...
                else if(line.contains("ssd:"))
                {
                    if(config.getNodeGroups().contains("ioHddSsdNodes"))
                    {
                        ...
                        for(j = i+1; j < lines.size(); j++)
                        {
                            ...
                            {
                                newLines.add(Utils.updateFileLine(line, "image", config.getIoHddSsdNodes().get("image")));
                            }
                            else if(line.contains("num_vms:"))
                            {
    ...
    static void bastionClusterProvisionExecute(JSch ssh, Configuration config, Server bastion, String authCommands)
    {
        ...
        String commands = authCommands +
                ...
                "deactivate;" +
//                "printf '[defaults]\nremote_user = " + config.getUserName() + "' > ~/.ansible.cfg\n;" +
    ...
    static void transferInstallationFiles2Bastion(JSch ssh, Configuration config, Server bastion, boolean replace)
    {
        System.out.println("Start transferring installation files to Bastion...");
        String commands;
        if(replace)
        {
            commands = "rm -r " + Utils.getFileNameFromPath(config.getXternFiles().get("installation")) + ";";
            Utils.sshExecutor(ssh, config.getUserName(), Utils.getServerPublicIp(bastion, config.getNetworkName()), commands);
        }
        Utils.sshCopier(ssh, config.getUserName(), Utils.getServerPublicIp(bastion, config.getNetworkName()),
                new String[]{Utils.getFileNameFromPath(config.getXternFiles().get("installation"))}, "",
                true, replace);
//        commands = "source " + Utils.getFileNameFromPath(config.getXternFiles().get("installation")) + "/_unpack.sh;";
//        Utils.sshExecutor(ssh, config.getUserName(), Utils.getServerPublicIp(bastion, config.getNetworkName()), commands);
        System.out.println("Installation files are transferred to Bastion.");
    }
